import React from 'react'

export default function Governance() {
    return (
        <div className="relative max-w-6xl mx-auto px-0 sm:px-6">
        <div>
            <h3 className="select-none text-left text-5xl bg-clip-text text-transparent bg-gradient-to-r from-blue-500 to-teal-400">Governance</h3></div>

            <div>
            <p>Liqwid is managed by a decentralized community of LQ token-holders and their delegates, who propose and vote on upgrades to the protocol or request project funding from the LiqwiDAO Treasury.
Participate in governance to develop the future of the Liqwid Protocol, all based on $LQ.
</p></div>
    </div>
    )
}
